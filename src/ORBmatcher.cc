...
namespace ORB_SLAM3
{
    ...
    int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th, const bool bFarPoints, const float thFarPoints)
    {
        ...
        if ( TrackingKernelController::searchLocalPointsKernelRunStatus == 0 ) {
            ...
            for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++) {
                ...
                int bestDist2 = h_bestDist2[iMP];
                int bestLevel = h_bestLevel[iMP];
                int bestLevel2 = h_bestLevel2[iMP];
                int bestIdx = h_bestIdx[iMP];
                int bestDistR = h_bestDistR[iMP];
                int bestDistR2 = h_bestDistR2[iMP];
                int bestLevelR = h_bestLevelR[iMP];
                int bestLevelR2 = h_bestLevelR2[iMP];
                int bestIdxR = h_bestIdxR[iMP];
                ...
                if(F.Nleft != -1 && pMP->mbTrackInViewR){
                    if(pMP->mnTrackScaleLevelR != -1){
                        if(bestDistR<=TH_HIGH)
                        {
                            if(bestLevelR==bestLevelR2 && bestDistR>mfNNratio*bestDistR2){
                                continue;
                            }

                            if(F.Nleft != -1 && F.mvRightToLeftMatch[bestIdxR] != -1){ //Also match with the stereo observation at right camera
                                F.mvpMapPoints[F.mvRightToLeftMatch[bestIdxR]] = pMP;
                                nmatches++;
                                left++;
                            }

                            F.mvpMapPoints[bestIdxR + F.Nleft]=pMP;
                            nmatches++;
                            right++;
                        }
                    }
                                       ...
                                       std::vector<MapPoint*> &vpMatched, std::vector<KeyFrame*> &vpMatchedKF, int th, float ratioHamming)
    {
        ...
        if(false){
            SearchByProjectionKernel kernel;
            nmatches = kernel.launch(pKF, Scw, vpPoints, vpPointsKFs, vpMatched, vpMatchedKF, th, ratioHamming);
        }
        else{
            // Get Calibration Parameters for later projection
            const float &fx = pKF->fx;
                                    ...
                                    int &numProjMatches, int &numProjOptMatches)
    {
        ...
        // ORBmatcher::mpKernel->mergedlaunch(pKF, vpPoints,
        //                     Scw, vpPointsKFs, vpMatched, vpMatchedKF, th, ratioHamming,
        //                     Scw1, vpMatched1, th1, ratioHamming1,
        //                     numProjMatches, numProjOptMatches);
    ...
    int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
    {
        ...
        if ( TrackingKernelController::poseEstimationKernelRunStatus == 0) {
            ...
            const Sophus::SE3f Tcw = CurrentFrame.GetPose();
            Eigen::Matrix4f transform_matrix = Tcw.matrix();
            const Eigen::Vector3f twc = Tcw.inverse().translation();
            ...
            for (int i=0; i<LastFrame.N; i++) {
                ORB_SLAM3::MapPoint* pMP = LastFrame.mvpMapPoints[i];
                int bestDist = h_bestDist[i];
                int bestIdx2 = h_bestIdx2[i];
                int bestDistR = h_bestDistR[i];
                int bestIdxR2 = h_bestIdxR2[i];
                ...
                if(bestDistR<=TH_HIGH)
                {
                    CurrentFrame.mvpMapPoints[bestIdxR2 + CurrentFrame.Nleft]=pMP;
                    nmatches++;
                    if(mbCheckOrientation)
                    {
                        cv::KeyPoint kpLF = (LastFrame.Nleft == -1) ? LastFrame.mvKeysUn[i]
                        ...
                        cv::KeyPoint kpCF = CurrentFrame.mvKeysRight[bestIdxR2];
                        ...
                        if(bin==HISTO_LENGTH)
                            bin=0;
                        assert(bin>=0 && bin<HISTO_LENGTH);
                        rotHist[bin].push_back(bestIdxR2  + CurrentFrame.Nleft);
                    }
                }
            }
